...
int main(object me, string arg)
{
	...
	if( ob->query("death/half_quest") < 0 ) { 
		if( level < 50 && ob->query("death/time")+86400 < time() ) {
			// ³¬¹ý24 Ð¡Ê±£¬¿Ï¶¨²»ÄÜrevive,Ò²Ã»ÓÐPenalty
			if( ob == me ) {
			...
			line+= sprintf(CYN"\n	ÄãµÄËÀÍöËðÊ§ÔÚ£²£´Ð¡Ê±ºó×Ô¶¯ÏûÊ§ÁË£¡\n"NOR);
		} else {
			line += sprintf(CYN"\n¾àÀëÉÏ´ÎËÀÍöÊ±¼äÒÑÓÐ%dÐ¡Ê±%d·ÖÖÓ£¬»áÓ°Ïì´óÔ¼%d¸öµ¥ÈËÌì»úÈÎÎñ¡£\n"NOR,
...
int basic_damage(object attacker)
{
	...
	d_race = d1 /10;
	if( attacker->query("national") == "ÃÉ¹Å×å") {
		d1 += d_race;
	}
	if( attacker->query("national") == "Âú×å") {
		d1 -= d_race;
	}
